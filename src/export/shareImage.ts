...
import type { HtmlTemplate } from './html';
 ...
 * is the thin UI shell on top. Lives in the export layer alongside the other
 * exporters; unlike the string-only exporters it needs the DOM (canvas), so it
 * is only ever called from a UI surface (the side panel or the full-tab view).
 */
